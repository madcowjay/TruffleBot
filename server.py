...
prepadding  =   int(config.get('experiment-parameters', 'prepadding')) # pulses of silence at the beginning
postpadding =   int(config.get('experiment-parameters', 'postpadding'))# pulses of silence at the end
...
print('\tprepadding:               {} pulses'.format(prepadding))
print('\tpostpadding:              {} pulses'.format(postpadding))
		...
		if not t_stop.is_set():
			...
			for ip in pe.trials[trial_name]['collectors'].keys():
				...
				if ip in pe.trials[trial_name]['transmitters'].keys():
					...
					pe.add_element_to_transmitter(trial_name, ip, 'Prepadding', prepadding)
					pe.add_element_to_transmitter(trial_name, ip, 'Postpadding', postpadding)
